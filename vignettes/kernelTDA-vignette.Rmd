...
Before showing how to use the functions contained in this package, we briefly recap how they are defined, starting from the most common distance between Persistence Diagrams, i.e. the $L_p$ $q-$Wasserstein distance.

Given two Persistence Diagrams $D_1$, $D_2$, we define the as 
...
```{r}
library(kernlab)
kmat = as.kernelMatrix(GSWkernel)
GSWclust = specc(kmat, centers = 2)
```
